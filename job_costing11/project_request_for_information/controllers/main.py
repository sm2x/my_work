...
class RequestInformation(http.Controller):
    ...
    def request_information_submitted(self, **post):
        ...
        if Partner:
            ...
            if len(attachment_list) > 0:
                group_msg = 'Customer has sent %s attachments to this Request Information. Name of attachments are: ' % (
                    len(attachment_list))
    ...
    def start_rating(self, **kw):
        ...
        customer_msg = _(
            request_information_id.partner_id.name+'has send this feedback rating is %s and comment is %s') % (
                       kw['star'], kw['comment'],)
...
class CustomerPortal(CustomerPortal):
    ...
    @http.route(['/my/request_informations', '/my/request_informations/page/<int:page>'], type='http', auth="user",
                website=True)
    ...
    @http.route(['/my/request_information/<model("request.information"):request_information>'], type='http',
    ...
    def my_ticket(self, request_information=None, **kw):
        ...
        return request.render("project_request_for_information.display_request_information",
                              {'request_information': request_information, 'user': request.env.user})
