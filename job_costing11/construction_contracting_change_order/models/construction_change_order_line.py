...
class ConstructionChngeOrderLine(models.Model):
    ...
    def _compute_amount(self):
        ...
        for line in self:
            ...
            taxes = line.tax_ids.compute_all(price, line.currency_id, line.quantity, product=line.product_id,
                                             partner=line.change_order_id.partner_id)
            line.update({
    ...
    def _get_display_price(self, product):
        ...
        price, rule_id = self.change_order_id.pricelist_id.get_product_price_rule(self.product_id, self.quantity or 1.0,
                                                                                  self.change_order_id.partner_id)
        ...
        for rec in self:
            if rec.product_id:
                ...
                if self.change_order_id.pricelist_id and self.change_order_id.partner_id:
                    ...
                    vals['sale_price'] = self.env['account.tax']._fix_tax_included_price_company(
                        self._get_display_price(product), product.taxes_id, self.tax_ids,
                        self.change_order_id.company_id)
                    rec.update(vals)
