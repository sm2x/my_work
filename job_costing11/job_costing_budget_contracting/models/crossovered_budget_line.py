    ...
    def _compute_actual_quantity(self):
        for rec in self:
            for line in rec.jobcost_line_id.purchase_order_line_ids:
                if rec.date_from and rec.date_to and line.order_id.date_order.date() >= rec.date_from and line.order_id.date_order.date() <= rec.date_to and line.order_id.state in [
                    'purchase', 'done']:
    ...
    def _compute_actual_vendor_quantity(self):
        for rec in self:
            for line in rec.jobcost_line_id.account_invoice_line_ids:
                if rec.date_from and rec.date_to and line.invoice_id.date_invoice >= rec.date_from and line.invoice_id.date_invoice <= rec.date_to and line.state in [
                    'open', 'paid']:
