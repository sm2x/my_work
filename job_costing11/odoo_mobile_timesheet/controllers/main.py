...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_portal_layout_values(self):
        ...
        timesheets_count = timesheets.sudo().search_count([
            ...
            ('project_id.portal_user_ids', 'child_of', [request.env.user.id])
        ])
        values.update({
            'timesheets_count': timesheets_count,
        })
    ...
    def portal_my_timesheet(self, page=1, sortby=None, **kw):
        if not (request.env.user.has_group('base.group_user') or request.env.user.has_group('base.group_portal')):
        ...
        timesheets = timesheets_obj.sudo().search(domain, order=order, limit=self._items_per_page,
                                                  offset=pager['offset'])
    ...
    def portal_add_timesheet(self, page=1, date_begin=None, date_end=None,
        ...
        project_ids = request.env['project.project'].sudo().search(
            [('portal_user_ids', 'child_of', [request.env.user.id]), ('is_close', '=', False)])
        task_ids = request.env['project.task'].sudo().search(
            [('portal_user_ids', 'child_of', [request.env.user.id]), ('stage_id.is_close', '=', False)])
    ...
    def create_new_timesheet(self, **kwargs):
        ...
        if not (request.env.user.has_group('base.group_user') or request.env.user.has_group(
                'base.group_portal')) or not kwargs:
            return request.render("odoo_mobile_timesheet.not_allowed")
        valse = {
        ...
        #            valse.update({'payment_status': kwargs.get('payment_status')})
        ...
        #        domain = [
        #            ('user_id', 'child_of', [request.env.user.id]), ('project_id.portal_user_ids','child_of', [request.env.user.id]), ('date', '=', kw.get('start_date'))]
    ...
    def update_timesheet(self, **kwargs):
        ...
        if kwargs.get('line_id'):
            ...
            if line_id:
                line_id.write(valse)
                return request.redirect(
                    "/odoo_timesheet_portal_user_employee/select_timesheet?start_date="+str(line_id.date))
    ...
    def select_timesheet(self, page=1, sortby=None, **kw):
        ...
        # return request.render("odoo_timesheet_portal_user_employee.select_timesheets", values)
