...
class website_account(website_account):
    ...
    def joborder_submitted(self, **post):
        ...
        if len(attachment_list) > 0:
            group_msg = _('Customer has sent %s attachments to this joborder. Name of attachments are: ') % (
                len(attachment_list))
    ...
    def my_joborders(self, page=1, date_begin=None, date_end=None, joborder=None, sortby=None, **kw):
        ...
        job_order_ids = request.env['project.task'].sudo().search(domain, limit=self._items_per_page,
                                                                  offset=pager['offset'])
        values.update({
            'date': date_begin,
            'date_end': date_end,
