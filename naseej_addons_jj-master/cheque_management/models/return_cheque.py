        ...
        if self.amount and self.amount > 0:
            line_ids = [(0, 0,
             {'journal_id': journal_id, 'account_id': cheque_obj.bank_name.account_id.id, 'name': '/',
              ...
              'currency_id': cheque_obj.currency_id.id, 'credit': self.amount}),
            (0, 0, {'journal_id': journal_id, 'account_id': cheque_obj.bank_name.return_account_id.id,
                    'name': 'Bank Charges', 'partner_id': cheque_obj.partner_id.id, 'amount_currency': cheque_obj.amount_currency or False,
                  'currency_id': cheque_obj.currency_id.id, 'debit': self.amount}),]
        line_ids.extend([
            (0, 0,
             {'journal_id': journal_id, 'account_id': cheque_obj.partner_account_id.id, 'name': '/',
              'amount_currency': -cheque_obj.amount_currency or False,
              'currency_id': cheque_obj.currency_id.id, 'credit': cheque_obj.amount}),
            (0, 0, {'journal_id': journal_id, 'account_id': cheque_obj.bank_name.account_id.id,
                    'name': cheque_obj.name, 'amount_currency': cheque_obj.amount_currency or False,
                    'currency_id': cheque_obj.currency_id.id, 'debit': cheque_obj.amount}),
            (0, 0,
             {'journal_id': journal_id, 'account_id': cheque_obj.bank_name.account_id.id, 'name': '/',
              'amount_currency': -cheque_obj.amount_currency or False,
              'currency_id': cheque_obj.currency_id.id, 'credit': cheque_obj.amount}),
